...
Production-расчёт площади работ по ЭППС 100 мм торец/борт фундаментной плиты:

```text
eps100_wall_insulation_area_m2 = явная площадь из проекта,
если она есть
```

Если проект не даёт отдельную площадь, сохраняется fallback:
...
Торец плиты, борт плиты и вертикальная поверхность плит фундамента в текущем MVP
считаются одной сущностью. Стены фундамента/ростверк сейчас не входят в этот расчёт.
