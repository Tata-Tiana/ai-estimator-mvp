...
def p(
    ...
    # resolver_hints semantics (enforced by resolver_engine, A4):
    #   positive_context — ALL patterns must match candidate text (AND logic)
    #   negative_context — ANY matching pattern rejects the candidate (OR logic)
    #   expected_unit / expected_kind — applied to value_candidates, not raw text
    #   aggregation: "pick_best" (default, scalar) | "collect_all" (list params)
    return {
        ...
        # LEGACY_DEPRECATED_REGEX_PATTERNS
        # These were used by the old direct-text parser flow (pre-A4).
        # A4 resolver_engine reads only resolver_hints — NOT this field.
        # Do not add project-specific quantities here.
        # Do not use this field in any new resolver, adapter, or engine.
...
EXPLICIT_PARAMETERS: dict[str, list[dict[str, Any]]] = {
    "earthworks": [
        p("sand_volume", "sand_base_volume_m3", "Песок", "м3", "parsed", True,
          ["Песок"], [r"Песок.*?Купл=0,95\s*(?P<value>\d+[,.]\d+)"],
          resolver_hints={
              "expected_unit": "m3",
              "expected_kind": "volume",
              "candidate_types": ["material_quantity", "label_value_quantity", "unknown_relevant_quantity", "table_quantity_row"],
              "positive_context": [r"песок"],
              "negative_context": [],
              # Купл=0,95 may appear in the same string as the volume value — it must
              # NOT reject the candidate. The resolver selects by unit/kind (m3/volume),
              # so the 0.95 coefficient (unitless) is simply ignored by value_candidates.
              "value_range": [1.0, 300.0],
          }),
        p("geotextile_area", "geotextile_area_m2", "Геотекстиль", "м2", "parsed", True,
          ["Геотекстиль"], [r"Геотекстиль.*?(?P<value>\d+[,.]?\d*)\s*м2"],
          resolver_hints={
              "expected_unit": "m2",
              "expected_kind": "area",
        ...
        p("planter_membrane_area", "foundation_slab.membrane_area_m2", "Профилированная мембрана PLANTER", "м2", "parsed", False,
          ["PLANTER"], [r"PLANTER\s*(?P<value>\d+[,.]?\d*)\s*м2"],
          resolver_hints={
              "expected_unit": "m2",
              "expected_kind": "area",
              "candidate_types": ["material_quantity", "label_value_quantity"],
              "positive_context": [r"PLANTER", r"мембран"],
              "value_range": [10.0, 1000.0],
          }),
        p("pit_area", "pit_area_m2", "Площадь котлована", "м2", "parsed", True,
          ["Площадь котлована", "котлован"], [r"[Пп]лощадь\s*котлован.*?(?P<value>\d+[,.]?\d*)\s*м2"],
          resolver_hints={
              "expected_unit": "m2",
              "expected_kind": "area",
              "candidate_types": ["label_value_quantity", "table_quantity_row", "unknown_relevant_quantity"],
              "positive_context": [r"площад", r"котлован"],
              "negative_context": [r"помещен", r"застройк", r"террас", r"кровл"],
              "value_range": [20.0, 5000.0],
          }),
        p("pit_excavation_depth", "pit_excavation_depth_m", "Глубина котлована", "м", "parsed", True,
          ...
          resolver_hints={
              ...
              "candidate_types": ["label_value_quantity", "table_quantity_row", "unknown_relevant_quantity"],
              "positive_context": [r"глубин", r"котлован"],
              "negative_context": [r"транше", r"трасс"],
              "value_range": [0.3, 5.0],
          }),
        p("trench_routes", "trench_routes", "Трассы коммуникаций (К1/К2/К3/В1/ЭО/Дренаж)", "-", "parsed", False,
          ["К1", "К2", "К3", "В1", "ЭО", "Дренаж"], [],
          resolver_hints={
              # Aggregation: collect ALL matching route_summary candidates, not one.
              # Each route becomes a dict with label, length, depth, volume fields.
              "aggregation": "collect_all",
              "candidate_types": ["route_summary"],
              "positive_context": [r"К[0-9]|В[0-9]|ЭО|дренаж|трасс"],
              "prefer_context": [r"ИТОГО", r"длина", r"м3"],
              "collect_fields": {
                  "label":  {"source": "subject_hint"},
                  "length": {"expected_unit": "linear_m", "fallback_unit": "m"},
                  "depth":  {"expected_unit": "m", "expected_kind": "elevation"},
                  "volume": {"expected_unit": "m3", "expected_kind": "volume"},
              },
          }),
        p("communications_pipe_items", "communications_pipe_items", "Трубы коммуникаций (ПНД/ПВХ/гофр)", "-", "parsed", False,
          ["труба", "ПНД", "ПВХ", "гофр"], [],
          resolver_hints={
              # Aggregation: collect ALL pipe_item candidates.
              # Ø/diameter is a characteristic of the pipe, not the quantity to resolve.
              # The resolver must NOT confuse diameter (mm) with pipe length (linear_m).
              "aggregation": "collect_all",
              "candidate_types": ["pipe_item"],
              "positive_context": [r"труб|ПНД|ПВХ|гофр"],
              "collect_fields": {
                  "name":        {"source": "subject_hint"},
                  "length":      {"expected_unit": "linear_m", "expected_kind": "linear_length"},
    ...
    "foundation_slab": [
        ...
        p("concrete_project_volume", "concrete_project_volume_m3", "Проектный объем бетона фундаментной плиты", "м3", "parsed", True,
          ["Бетон", "м3"], [r"Бетон\s*В\s*22.*?(?P<value>\d+[,.]?\d*)\s*м3"],
          resolver_hints={
              "expected_unit": "m3",
              "expected_kind": "volume",
              "candidate_types": ["material_quantity", "table_quantity_row"],
              "positive_context": [r"бетон"],
              # Concrete grade tokens (W6, F150, П4) appear in the same string as
              # the volume — they must NOT disqualify it via negative_context.
              # Instead the resolver should prefer the m3 value_candidate, not the
              # grade tokens. No negative_context needed here.
              "value_range": [5.0, 500.0],
          }),
        p("eps_50_under_slab_volume", "eps50_under_slab_volume_m3", "ЭППС 50 мм, низ плиты", "м3", "parsed", True,
          ["ЭППС 50 мм", "низ"], [r"ЭППС\s*50\s*мм\s*\(низ\)\s*(?P<value>\d+[,.]\d+)"],
          resolver_hints={
              "expected_unit": "m3",
              "expected_kind": "volume",
              "candidate_types": ["material_quantity", "label_value_quantity"],
              "positive_context": [r"ЭППС", r"50"],
              "negative_context": [r"100\s*мм", r"торец"],
              "value_range": [0.1, 100.0],
          }),
        p("eps_100_edge_volume", "eps100_edge_volume_m3", "ЭППС 100 мм, торец", "м3", "parsed", False,
          ["ЭППС 100 мм", "торец"], [r"ЭППС\s*100\s*мм\s*\(торец\)\s*(?P<value>\d+[,.]\d+)"],
          resolver_hints={
              "expected_unit": "m3",
              "expected_kind": "volume",
              "candidate_types": ["material_quantity", "label_value_quantity"],
              "positive_context": [r"ЭППС", r"100"],
              "negative_context": [r"50\s*мм", r"низ"],
        ...
        p("slab_side_formwork_area", "slab_side_formwork_area_m2", "Площадь опалубки бортов фундаментной плиты по спецификации", "м2", "parsed", True,
          ["опалубка", "борта"], [r"Опалубк[аи].*?борт.*?(?P<value>\d+[,.]?\d*)\s*м2"],
          resolver_hints={
              "expected_unit": "m2",
              "expected_kind": "area",
              "candidate_types": ["material_quantity", "label_value_quantity", "table_quantity_row"],
              "positive_context": [r"опалубк"],
              "negative_context": [r"фанер", r"плит"],
        ...
        p("thermal_insert_50_length", "thermal_insert_50_length_m", "Длина термовставок 50 мм по спецификации", "мп", "parsed", True,
          ["термовставки 50 мм"], [r"Термовставк[аи]\s*50\s*мм.*?(?P<value>\d+[,.]?\d*)\s*м\.?п"],
          resolver_hints={
              "expected_unit": "linear_m",
              "expected_kind": "linear_length",
              "candidate_types": ["material_quantity", "label_value_quantity"],
              "positive_context": [r"термовставк", r"50"],
              "negative_context": [r"100\s*мм"],
              "value_range": [1.0, 300.0],
          }),
        p("thermal_insert_100_length", "thermal_insert_100_length_m", "Длина термовставок 100 мм по спецификации", "мп", "parsed", True,
          ["термовставки 100 мм"], [r"Термовставк[аи]\s*100\s*мм.*?(?P<value>\d+[,.]?\d*)\s*м\.?п"],
          resolver_hints={
              "expected_unit": "linear_m",
              "expected_kind": "linear_length",
              "candidate_types": ["material_quantity", "label_value_quantity"],
              "positive_context": [r"термовставк", r"100"],
              "negative_context": [r"50\s*мм"],
              "value_range": [1.0, 300.0],
          }),
        p("thermal_insert_50_material_spec_qty", "thermal_insert_50_material_spec_qty", "Материал термовставок 50 мм по спецификации", "м3", "parsed", True,
          ["термовставки 50 мм", "м3"], [r"Термовставк[аи]\s*50\s*мм.*?(?P<value>\d+[,.]?\d*)\s*м3"],
          resolver_hints={
              "expected_unit": "m3",
              "expected_kind": "volume",
              "candidate_types": ["material_quantity", "label_value_quantity"],
              "positive_context": [r"термовставк", r"50"],
              "negative_context": [r"100\s*мм"],
              "value_range": [0.01, 10.0],
          }),
        p("thermal_insert_100_material_spec_qty", "thermal_insert_100_material_spec_qty", "Материал термовставок 100 мм по спецификации", "м3", "parsed", True,
          ["термовставки 100 мм", "м3"], [r"Термовставк[аи]\s*100\s*мм.*?(?P<value>\d+[,.]?\d*)\s*м3"],
          resolver_hints={
              "expected_unit": "m3",
              "expected_kind": "volume",
              "candidate_types": ["material_quantity", "label_value_quantity"],
        ...
        p("rebar_a500c_d12_main_grid_weight", "rebar_items[1].weight_parts_kg", "Арматура A500C d12, сетка фундаментной плиты", "кг", "parsed", True,
          ["ф12 А500С", "сетка"], [r"ф12\s*А500С\s*\(сетка\)\s*(?P<value>\d+[,.]?\d*)\s*кг"],
          resolver_hints={
              "expected_unit": "kg",
              "expected_kind": "weight",
              "candidate_types": ["material_quantity", "table_quantity_row"],
              "positive_context": [r"ф12", r"А500С", r"сетк"],
              "negative_context": [r"термовставк", r"ар-я\s*сетка", r"перемычк"],
              "value_range": [50.0, 5000.0],
          }),
        p("rebar_a500c_d12_thermal_insert_weight", "rebar_items[1].weight_parts_kg", "Арматура A500C d12, сетка термовставок", "кг", "parsed", True,
          ["ф12 А500С", "термовстав"], [r"ф12\s*А500С\s*\(ар-я сетка\)\s*кг\s*(?P<value>\d+[,.]?\d*)"],
          resolver_hints={
              "expected_unit": "kg",
              "expected_kind": "weight",
              "candidate_types": ["material_quantity", "table_quantity_row"],
              "positive_context": [r"ф12", r"А500С", r"ар-я\s*сетка|термовставк"],
              "negative_context": [r"\(сетка\)", r"перемычк"],
              "value_range": [10.0, 2000.0],
          }),
        p("rebar_a500c_d10_weight", "rebar_items[2].weight_parts_kg", "Арматура A500C d10", "кг", "parsed", True,
          ["ф10 А500С"], [r"ф10\s*А500С.*?(?P<value>\d+[,.]?\d*)\s*кг"],
          resolver_hints={
              "expected_unit": "kg",
              "expected_kind": "weight",
              "candidate_types": ["material_quantity", "table_quantity_row"],
              "positive_context": [r"ф10", r"А500С"],
              "negative_context": [r"армирование\s*кладки", r"перемычк"],
              "value_range": [10.0, 3000.0],
          }),
        p("rebar_a500c_d16_thermal_insert_weight", "rebar_items[0].weight_parts_kg", "Арматура A500C d16 для термовставок", "кг", "parsed", True,
          ["ф16 А500С"], [r"ф16\s*А500С.*?кг\s*(?P<value>\d+[,.]?\d*)"],
          resolver_hints={
              "expected_unit": "kg",
              "expected_kind": "weight",
              "candidate_types": ["material_quantity", "table_quantity_row"],
              "positive_context": [r"ф16", r"А500С"],
              "value_range": [10.0, 2000.0],
          }),
        p("rebar_a240_d6_thermal_insert_weight", "rebar_items[3].weight_parts_kg", "Арматура A240 d6 для термовставок", "кг", "parsed", True,
          ["ф6 А240"], [r"(?P<value>\d+[,.]?\d*)\s*ф6\s*А240|ф6\s*А240.*?кг\s*(?P<value2>\d+[,.]?\d*)"],
          resolver_hints={
              "expected_unit": "kg",
              "expected_kind": "weight",
              "candidate_types": ["material_quantity", "table_quantity_row"],
              "positive_context": [r"ф6", r"А240"],
              "negative_context": [r"перемычк"],
              "value_range": [5.0, 500.0],
          }),
        p("sand_volume", "earthworks.sand_base_volume_m3", "Песок", "м3", "parsed", True,
          ["Песок"], [r"Песок.*?Купл=0,95\s*(?P<value>\d+[,.]\d+)"],
          resolver_hints={
              "expected_unit": "m3", "expected_kind": "volume",
              "candidate_types": ["material_quantity", "label_value_quantity", "unknown_relevant_quantity", "table_quantity_row"],
              "positive_context": [r"песок"], "negative_context": [], "value_range": [1.0, 300.0],
          }),
        p("geotextile_area", "earthworks.geotextile_area_m2", "Геотекстиль", "м2", "parsed", True,
          ["Геотекстиль"], [r"Геотекстиль.*?(?P<value>\d+[,.]?\d*)\s*м2"],
          resolver_hints={
              "expected_unit": "m2", "expected_kind": "area",
              "candidate_types": ["material_quantity", "label_value_quantity"],
              "positive_context": [r"геотекстил"], "value_range": [10.0, 2000.0],
          }),
        p("planter_membrane_area", "membrane_area_m2", "Профилированная мембрана PLANTER", "м2", "parsed", True,
          ["PLANTER"], [r"PLANTER\s*(?P<value>\d+[,.]?\d*)\s*м2"],
          resolver_hints={
              "expected_unit": "m2", "expected_kind": "area",
              "candidate_types": ["material_quantity", "label_value_quantity"],
              "positive_context": [r"PLANTER", r"мембран"], "value_range": [10.0, 1000.0],
          }),
    ...
    "waterproofing": [
        ...
        p("eps_100_edge_volume", "eps100_wall_volume_m3", "ЭППС 100 мм, торец/вертикальное утепление", "м3", "parsed", True,
          ["ЭППС 100 мм", "торец"], [r"ЭППС\s*100\s*мм\s*\(торец\)\s*(?P<value>\d+[,.]\d+)"],
          resolver_hints={
              "expected_unit": "m3",
              "expected_kind": "volume",
              "candidate_types": ["material_quantity", "label_value_quantity"],
              "positive_context": [r"ЭППС", r"100"],
              "negative_context": [r"50\s*мм", r"низ\s*плит"],
              "value_range": [0.1, 50.0],
          }),
    ],
    "load_bearing_walls_lintels": [
        p("gas_block_d400_volume", "main_wall_gas_block_400_spec_volume_m3", "Газобетонный блок 600х400х250", "м3", "parsed", True,
          ["Газобетонный блок 600х400х250"], [r"Газобетонный блок\s*600х400х250\s*(?P<value>\d+[,.]\d+)\s*м3"],
          resolver_hints={
              "expected_unit": "m3", "expected_kind": "volume",
              "candidate_types": ["material_quantity", "table_quantity_row"],
              "positive_context": [r"газобетон", r"400"],
              "negative_context": [r"600.{1,6}250.{1,6}250", r"600.{1,6}150.{1,6}250"],
              "value_range": [1.0, 200.0],
          }),
        # DEFERRED: gas_block_d500_250 and d500_150 are extracted by table row position —
        # the regex captures two adjacent cells from the same row. Generic resolver
        # cannot distinguish them without knowing the exact table layout.
        ...
        p("masonry_rebar_a500_d10_weight", "rebar_a500_d10_source_weight_kg", "Арматура ф10 А500С для кладки", "кг", "parsed", True,
          ["ф10 А500С", "армирование кладки"], [r"ф10\s*А500С\s*\(армирование кладки\)\s*(?P<value>\d+[,.]?\d*)\s*кг"],
          resolver_hints={
              "expected_unit": "kg", "expected_kind": "weight",
              "candidate_types": ["material_quantity", "table_quantity_row"],
              "positive_context": [r"ф10", r"А500С", r"армирован.{0,10}кладк"],
              "negative_context": [r"перемычк", r"фундамент", r"перекрыти"],
              "value_range": [10.0, 2000.0],
          }),
        p("lintel_concrete_volume", "lintel_concrete_volume_m3", "Бетон перемычек", "м3", "parsed", True,
          ["Бетон В-22,5"], [r"Бетон\s*В-22,5\s*(?P<value>\d+[,.]\d+)\s*м3"],
          resolver_hints={
              "expected_unit": "m3", "expected_kind": "volume",
              "candidate_types": ["material_quantity", "table_quantity_row"],
              "positive_context": [r"бетон", r"перемычк"],
              "negative_context": [r"фундамент", r"перекрыти", r"плит"],
              "value_range": [0.1, 20.0],
          }),
        p("lintel_rebar_a500_d12_weight", "lintel_rebar_items[0].weight_kg", "Арматура ф12 А500С для перемычек", "кг", "parsed", True,
          ["ф12 А500С"], [r"ф12\s*А500С\s*кг\s*(?P<value>\d+[,.]?\d*)"],
          resolver_hints={
              "expected_unit": "kg", "expected_kind": "weight",
              "candidate_types": ["material_quantity", "table_quantity_row"],
              "positive_context": [r"ф12", r"А500С", r"перемычк"],
              "negative_context": [r"сетк", r"фундамент", r"перекрыти"],
              "value_range": [5.0, 500.0],
          }),
        p("lintel_rebar_a240_d6_weight", "lintel_rebar_items[1].weight_kg", "Арматура ф6 А240 для перемычек", "кг", "parsed", True,
          ["ф6 А240"], [r"ф6\s*А240\s*\(хомуты\)\s*(?P<value>\d+[,.]\d*)\s*кг"],
          resolver_hints={
              "expected_unit": "kg", "expected_kind": "weight",
              "candidate_types": ["material_quantity", "table_quantity_row"],
              "positive_context": [r"ф6", r"А240", r"хомут|перемычк"],
    ...
    "floor_slab_1": [
        p("floor_slab_1_concrete_volume", "total_concrete_volume_from_spec_m3", "Бетон плиты перекрытия +3.480", "м3", "parsed", True,
          ["40,53 м3"], [r"(?P<value>40[,.]53)\s*м3"],
          resolver_hints={
              "expected_unit": "m3", "expected_kind": "volume",
              "candidate_types": ["material_quantity", "table_quantity_row"],
              "positive_context": [r"бетон", r"перекрыти"],
              "negative_context": [r"фундамент", r"перемычк"],
              "value_range": [10.0, 200.0],
          }),
        p("floor_slab_1_eps100_volume", "total_eps_volume_from_spec_m3", "ЭППС 100 мм торец и низ плиты", "м3", "parsed", True,
          ["ЭППС 100 мм"], [r"ЭППС\s*100\s*мм.*?(?P<value>\d+[,.]\d+)\s*ГОСТ"],
          resolver_hints={
              "expected_unit": "m3", "expected_kind": "volume",
              "candidate_types": ["material_quantity", "label_value_quantity"],
              "positive_context": [r"ЭППС", r"100"],
              "negative_context": [r"50\s*мм", r"фундамент", r"термовставк"],
              "value_range": [0.5, 50.0],
          }),
        p("floor_slab_1_rebar_a500_d10_weight", "rebar_items[3].source_weight_parts_kg", "Арматура ф10 А500С", "кг", "parsed", True,
          ["ф10 А500С"], [r"ф10\s*А500С.*?кг\s*(?P<value>\d+[,.]?\d*)"],
          resolver_hints={
              "expected_unit": "kg", "expected_kind": "weight",
              "candidate_types": ["material_quantity", "table_quantity_row"],
              "positive_context": [r"ф10", r"А500С"],
              "negative_context": [r"армирован.{0,10}кладк", r"перемычк", r"фундамент"],
              "value_range": [10.0, 3000.0],
          }),
        p("floor_slab_1_rebar_a500_d12_weight", "rebar_items[2].source_weight_kg", "Арматура ф12 А500С", "кг", "parsed", True,
          ["ф12 А500С"], [r"ф12\s*А500С.*?(?P<value>\d+[,.]?\d*)\s*кг"],
          resolver_hints={
              "expected_unit": "kg", "expected_kind": "weight",
              "candidate_types": ["material_quantity", "table_quantity_row"],
              "positive_context": [r"ф12", r"А500С"],
              "negative_context": [r"перемычк", r"кладк", r"фундамент"],
              "value_range": [10.0, 3000.0],
          }),
        # DEFERRED: d25/d16/d6 are extracted from a single multi-column table row where
        # all three values (162, 52.04, 15) appear together. The generic resolver
        # cannot distinguish them without knowing expected values in advance.
        ...
        p("floor_slab_1_rebar_a240_d8_weight", "rebar_items[4].source_weight_kg", "Арматура ф8 А240", "кг", "parsed", True,
          ["ф8 А240"], [r"ф8\s*А240.*?(?P<value>\d+[,.]?\d*)\s*кг"],
          resolver_hints={
              "expected_unit": "kg", "expected_kind": "weight",
              "candidate_types": ["material_quantity", "table_quantity_row"],
              "positive_context": [r"ф8", r"А240"],
              "negative_context": [r"перемычк", r"кладк"],
              "value_range": [5.0, 500.0],
          }),
    ],
    "floor_slab_2": [
        p("floor_slab_2_concrete_volume", "concrete_placing_volume_m3", "Бетон плиты перекрытия +3.680/+4.680", "м3", "parsed", True,
          ["16,5 м3"], [r"(?P<value>16[,.]5)\s*м3"],
          resolver_hints={
              "expected_unit": "m3", "expected_kind": "volume",
              "candidate_types": ["material_quantity", "table_quantity_row"],
              "positive_context": [r"бетон", r"перекрыти"],
        ...
        p("floor_slab_2_eps100_edge_volume", "eps100_edge_volume_m3", "ЭППС 100 мм торец плиты", "м3", "parsed", False,
          ["ЭППС 100 мм"], [r"ЭППС\s*100\s*мм\s*\(торец плиты\)\s*(?P<value>\d+[,.]\d+)"],
          resolver_hints={
              "expected_unit": "m3", "expected_kind": "volume",
              "candidate_types": ["material_quantity", "label_value_quantity"],
              "positive_context": [r"ЭППС", r"100"],
              "negative_context": [r"50\s*мм", r"фундамент"],
              "value_range": [0.1, 20.0],
          }),
        p("floor_slab_2_rebar_a500_d10_weight_main", "rebar_items[2].source_weight_kg", "Арматура ф10 А500С, основная", "кг", "parsed", True,
          ["ф10 А500С"], [r"ф10\s*А500С.*?кг\s*(?P<value>1500)"],
          resolver_hints={
              "expected_unit": "kg", "expected_kind": "weight",
              "candidate_types": ["material_quantity", "table_quantity_row"],
              "positive_context": [r"ф10", r"А500С"],
              "negative_context": [r"армирован.{0,10}кладк", r"перемычк", r"фундамент"],
              # Broad sanity range — disambiguation is done by positive_context, not range.
              "value_range": [50.0, 5000.0],
          }),
        # DEFERRED: d10 extras are on different pages (25, 26) with the same diameter
        # as the main item. Generic resolver cannot distinguish by page number.
        ...
        p("floor_slab_2_rebar_a500_d12_weight", "rebar_items[1].source_weight_kg", "Арматура ф12 А500С", "кг", "parsed", True,
          ["ф12 А500С"], [r"ф12\s*А500С.*?(?P<value>22)\s*кг"],
          resolver_hints={
              "expected_unit": "kg", "expected_kind": "weight",
              "candidate_types": ["material_quantity", "table_quantity_row"],
              "positive_context": [r"ф12", r"А500С"],
        ...
        p("floor_slab_2_rebar_a500_d16_weight", "rebar_items[0].source_weight_kg", "Арматура ф16 А500С", "кг", "parsed", True,
          ["ф16 А500С"], [r"ф16\s*А500С\s*кг\s*(?P<value>142[,.]4)"],
          resolver_hints={
              "expected_unit": "kg", "expected_kind": "weight",
              "candidate_types": ["material_quantity", "table_quantity_row"],
              "positive_context": [r"ф16", r"А500С"],
    ...
    "flat_roof": [
        p("project_spec_roof_area", "project_spec_roof_area_m2", "Площадь кровли по спецификации", "м2", "parsed", False,
          ["Пароизоляционный слой"], [r"Пароизоляционный слой.*?(?P<value>294)\s*м3"],
          resolver_hints={
              "expected_unit": "m2", "expected_kind": "area",
              "candidate_types": ["label_value_quantity", "table_quantity_row", "material_quantity"],
              "positive_context": [r"кровл"],
              "negative_context": [r"стен", r"парапет", r"фасад", r"фундамент"],
              "value_range": [50.0, 2000.0],
          }),
        p("roof_eps_main_volume", "eps100_supplier_volume_control_m3", "ЭППС кровли по спецификации", "м3", "parsed", False,
          ["ЭППС ТехноНИКОЛЬ"], [r"ЭППС\s*ТехноНИКОЛЬ.*?(?P<value>29[,.]4)"],
          resolver_hints={
              "expected_unit": "m3", "expected_kind": "volume",
              "candidate_types": ["material_quantity", "table_quantity_row"],
              "positive_context": [r"ЭППС", r"ТехноНИКОЛЬ|кровл"],
              "negative_context": [r"фундамент", r"термовставк", r"перекрыти"],
              "value_range": [1.0, 100.0],
          }),
        p("roof_pvc_membrane_area", "pvc_membrane_area_control_m2", "Полимерная мембрана LOGICROOF V-RP", "м2", "parsed", False,
          ["LOGICROOF"], [r"Полимерная мембрана\s*LOGICROOF\s*V-RP\s*(?P<value>\d+[,.]\d+)"],
          resolver_hints={
              "expected_unit": "m2", "expected_kind": "area",
              "candidate_types": ["material_quantity", "table_quantity_row"],
              "positive_context": [r"LOGICROOF|мембран", r"кровл|ПВХ"],
              "value_range": [50.0, 1500.0],
          }),
        p("roof_internal_drains_count", "internal_roof_drains_count", "Воронки внутреннего водостока", "шт", "parsed", True,
          ["внутреннего водостока"], [r"воронкой\s*внутреннего водостока\s*шт\s*(?P<value>\d+)"],
          resolver_hints={
              "expected_unit": "pcs", "expected_kind": "quantity",
              "candidate_types": ["material_quantity", "table_quantity_row", "pipe_piece_qty"],
              "positive_context": [r"воронк", r"внутренн"],
              "negative_context": [r"парапет"],
              "value_range": [1.0, 20.0],
          }),
        p("roof_parapet_drains_count", "parapet_roof_drains_count", "Парапетные воронки", "шт", "parsed", True,
          ["Парапетная воронка"], [r"Парапетная воронка\s*шт\s*(?P<value>\d+)"],
          resolver_hints={
              "expected_unit": "pcs", "expected_kind": "quantity",
              "candidate_types": ["material_quantity", "table_quantity_row", "pipe_piece_qty"],
              "positive_context": [r"воронк", r"парапет"],
              "negative_context": [r"внутренн"],
              "value_range": [1.0, 20.0],
          }),
    ...
    "schiedel_vent_channels": [
        ...
        p("schiedel_gas_block_150_volume", "load_bearing_walls_lintels.vent_chimney_gas_block_spec_volume_m3", "Газобетонный блок 600х150х250 для вентканалов", "м3", "parsed", False,
          ["Газобетонный блок 600х150х250"], [r"Газобетонный блок\s*600х150х250\s*(?P<value>\d+[,.]\d+)\s*м3"],
          resolver_hints={
              "expected_unit": "m3", "expected_kind": "volume",
              "candidate_types": ["material_quantity", "table_quantity_row"],
