...
def main() -> None:
    ...
    while True:
        ...
        try:
            ...
            if choice == "1":
                handle_text(parser, resolved_path)
            elif choice == "2":
                handle_blocks(parser, resolved_path)
            elif choice == "3":
                handle_tables(parser, resolved_path)
            elif choice == "4":
                handle_full_run(parser, resolved_path)
            else:
...
def handle_text(parser: PDFParser, pdf_path: Path) -> None:
    text_pages = parser.extract_text_pymupdf(pdf_path)
    result_dir, summary = parser.save_results(pdf_path, text_pages=text_pages)
    print(f"Страниц текста: {len(text_pages)}")
    print(f"Символов текста: {summary['text_characters']}")
    print(f"Папка результата: {result_dir}")


def handle_blocks(parser: PDFParser, pdf_path: Path) -> None:
    blocks = parser.extract_blocks_pymupdf(pdf_path)
    result_dir, summary = parser.save_results(pdf_path, blocks=blocks)
...
def handle_tables(parser: PDFParser, pdf_path: Path) -> None:
    tables = parser.extract_tables_pdfplumber(pdf_path)
    result_dir, summary = parser.save_results(pdf_path, tables=tables)
    print(f"Найдено таблиц: {summary['tables_found']}")
    print(f"Страницы с таблицами: {summary['pages_with_tables']}")
    print(f"Папка результата: {result_dir}")


def handle_full_run(parser: PDFParser, pdf_path: Path) -> None:
    text_pages = parser.extract_text_pymupdf(pdf_path)
    blocks = parser.extract_blocks_pymupdf(pdf_path)
    tables = parser.extract_tables_pdfplumber(pdf_path)
