...
**Обновление 2026-08-06: `gas_block_wall_hole_drilling` убран из контракта целиком.**
Разбирая листе 02 ТРЦ (пользователь хотела ноль "красных" цен), нашли
`gas_block_wall_hole_drilling_rate` без цены в реестре Елены — ни разу не
встречается ни в одной из 3 реальных смет (ТРЦ/АРК/ЮСВ). Проверила заметку
`open_question_gas_block_wall_hole_drilling.md`-типа: Elena 2026-07-30 уже
подтверждала, что это редкий частный случай ("особый проект"), не стандартная
работа. Вместо того чтобы искать/выдумывать цену для строки, которая может
никогда не понадобиться, пользователь решила убрать поле полностью: "давай
пробивку отверстий вообще уберем". Удалены 3 записи из
`sections/flat_roof/section_contract.yaml` — supplier_input
`gas_block_wall_holes_count`, price_key `gas_block_wall_hole_drilling_rate`,
estimate_line `gas_block_wall_hole_drilling` (было `enabled_by_default: false`,
т.е. и раньше не участвовало в стандартном выводе — удаление ничего не меняет
для обычных проектов, просто закрывает специальный случай, у которого никогда не
было реальной цены). Код `flat_roof_calculator.py` НЕ трогали — его собственная
ветка `if gas_block_wall_holes_count > 0: ...` остаётся мёртвым, но рабочим кодом
(на вход просто никогда больше не придёт этот ключ через адаптер/контракт, так
безопаснее, чем трогать протестированный калькулятор ради удаления мёртвого
кода). Адаптер (`review_to_calculator/sections/flat_roof/build_input.py`) правки
не требовал — поле уже проходило через общий `continue` для необязательных
supplier_inputs, только обновлён комментарий, перечисляющий скрытые
legacy-поля. Если когда-нибудь появится реальный проект с газоблочными стенами
и нужной пробивкой отверстий — придётся заново добавить все 3 записи и найти
цену в актуальном прайсе Елены на тот момент.
