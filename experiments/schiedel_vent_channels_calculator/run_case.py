...
def build_markdown(result: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        f"- schiedel_channel_items: `{result['inputs'].get('schiedel_channel_items', [])}`",
        f"- schiedel_delivery_trips: `{result['inputs']['schiedel_delivery_trips']}`",
        "",
        "## Важные правила",
        "",
        "- Кладка вентканалов — одна работа независимо от набора типов каналов (мп).",
        "- Количество вентканалов по типам (1х/2х/3х/4х/CVENT) берётся из schiedel_channel_items —",
        "  из спецификации проекта или, если PDF не даёт готовое количество в штуках, вручную от сметчицы.",
        "- Бренд всегда Schiedel, даже если в проекте название не указано.",
        "- Количество шахт/узлов на плане не считается и не используется — только количество модулей.",
        "- Расходные материалы считаются как процент от прямой базы (`consumables_rate`).",
