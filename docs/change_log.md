...
### 2026-05-26 — Добавлен вход и отчёт meeting analysis по плоской кровле

- Ветка: `feature/ai-project-card`
- Коммит: этот docs-коммит

Что добавлено:

- создана входная папка:
  - `experiments/meeting_analysis/input/2026-05-22_flat_roof/`;
- прогнан meeting analysis по 2 расшифровкам и 4 скриншотам;
- локальный результат сохранён в:
  - `data/output/meeting_analysis/2026-05-22_1103_flat_roof/`;
- создан docs-отчёт:
  - `docs/report_flat_roof_meeting_analysis.md`;
- обновлены инструкции и handoff:
  - `README.md`;
...
Ключевые правила, извлечённые из созвона:

- площадь кровли требует ручной проверки;
- мембраны/плёнки считаются с коэффициентом нахлёста `1.15`;
- утеплитель считается через площадь, толщину и коэффициент отходов `1.03`;
- геотекстиль по парапетам упоминается с коэффициентом `1.10`;
- рулонные материалы и рейки округляются вверх;
- воронки, вентканалы, парапеты, примыкания и уклоны требуют ручной проверки;
- редкие элементы вроде поликарбоната пока лучше оставлять ручными строками.

Что ещё не сделано:

- калькулятор плоской кровли;
- `input.json` / `expected.json` для кровли;
- Excel export для кровли;
