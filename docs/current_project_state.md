...
### 6.7. УНИКМА: `experiments/unikma_api_tests/`
...
1. Закоммитить текущую контрольную точку: проектные папки, новые калькуляторы, отчёты и обновлённую документацию.
...
3. Продолжить следующий раздел сметы отдельным экспериментальным калькулятором.
